...
@section('page-title', trans("laravel-job-status::progress.title", ['name' => $status->type]))

@push('breadcrumb')
    <li class="breadcrumb-item">{{ trans("laravel-job-status::progress.title", ['name' => $status->type]) }}</li>
@endpush

@section('content')
    @if ($status->status == 'queued')
        <div class="alert alert-info">
            {{ trans("laravel-job-status::progress.submitted", ['name' => $status->type]) }}
        </div>
    @elseif ($status->status == 'executing')
        <div class="alert alert-info">
            {{ trans("laravel-job-status::progress.processing", ['name' => $status->type]) }}
        </div>
    @elseif ($status->status == 'finished')
        @if (isset($status->output['success_rows']))
            @if ($status->output['failed_rows'] == 0)
                <div class="alert alert-success">
                    <p>{{ trans("laravel-job-status::progress.finished", ['name' => $status->type]) }}</p>
            ...
            @elseif ($status->output['success_rows'] == 0)
                <div class="alert alert-danger">
                    <p>{{ trans("laravel-job-status::progress.all_failed", ['name' => $status->type]) }}</p>
                    <p>{{ sprintf(trans("laravel-job-status::progress.result_numbers"), $status->output['total_rows'], $status->output['success_rows'], $status->output['skipped_rows'], $status->output['failed_rows']) }}</p>
                </div>
            @else
                <div class="alert alert-warning">
                    <p>{{ trans("laravel-job-status::progress.partially_finished", ['name' => $status->type]) }}</p>
                    <p>{{ sprintf(trans("laravel-job-status::progress.result_numbers"), $status->output['total_rows'], $status->output['success_rows'], $status->output['skipped_rows'], $status->output['failed_rows']) }}</p>
                </div>
            @endif
        @else
